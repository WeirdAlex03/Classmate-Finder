...
class MissingDataError extends ValidationError {
	...
	constructor(fieldName: string, fieldId: string) {
...
function validateFields(): boolean {
	...
	let hasName = false; let hasClass = false;
 ...
 * 
 */
function validateName(nameField: HTMLFieldSetElement) {

	const nameInput = nameField.querySelector("input[name=name]") as HTMLInputElement;
	const discordInput = nameField.querySelector("input[name=discord]") as HTMLInputElement;
	const passwordInput = nameField.querySelector("input[name=password]") as HTMLInputElement;
 ...
 * @throws {ValidationError} if a field input is invalid or missing altogether
 */
function validateClass(classField: HTMLFieldSetElement) {

	const classNum = classField.id.charAt(classField.id.length - 1);

	const departmentInput = classField.querySelector(`input[name=c${classNum}d]`) as HTMLInputElement;
	const courseInput = classField.querySelector(`input[name=c${classNum}c]`) as HTMLInputElement;
	const sectionInput = classField.querySelector(`input[name=c${classNum}s]`) as HTMLInputElement;
	const labInput = classField.querySelector(`input[name=c${classNum}l]`) as HTMLInputElement;
	 ...
	 */
	if (
		departmentInput.value !== "" || courseInput.value !== "" || sectionInput.value !== "" || labInput.value !== ""
	   ) {
